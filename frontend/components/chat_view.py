...
def render_chat_view(
    ...
    c_back, c_title, c_tool = st.columns([0.22, 0.52, 0.26])
    with c_back:
    ...
    with c_title:
        ...
        <div style="text-align: center; padding-top: 4px;">
            <div style="font-weight: 800; font-size: 1.15rem; color: #ffffff;">TastyTalk 🥑</div>
            <div style="font-size: 0.75rem; color: #34d399;">● Online Food & Nutrition Coach</div>
        </div>
    ...
    if len(st.session_state.messages) == 0 and not prompt_to_send:
        ...
        <div style="text-align: center; padding: 25px 15px;">
            <div style="width: 70px; height: 70px; border-radius: 50%; overflow: hidden; border: 2.5px solid #10b981; margin: 0 auto 12px auto;">
                <img src="{mascot_src}" style="width: 100%; height: 100%; object-fit: cover;" />
            </div>
            <div style="font-weight: 700; font-size: 1.3rem; color: #ffffff;">Ask TastyTalk Anything! 🥑</div>
            <div style="font-size: 0.88rem; color: #94a3b8; max-width: 400px; margin: 6px auto 16px auto;">
                Get delicious healthy recipes, instant calorie checks, and personalized meal architectures for your {preference} diet!
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Render Styled Conversation Bubbles (Reference Image 2 style)
    ...
    # Quick Suggestion Chips (Reference Image 3 style)
    if len(st.session_state.messages) > 0 and not prompt_to_send:
        st.markdown("""
        <div style="font-size: 0.75rem; color: #94a3b8; font-weight: 700; text-transform: uppercase; margin: 12px 0 6px 0;">
        ...
        with chip2:
            ...
            if st.button("⏱️ Intermittent Fasting Schedule", key="sugg_fast", use_container_width=True):
